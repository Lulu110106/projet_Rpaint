...
impl eframe::App for PaintApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        while let Ok(line) = self.incoming_draw_rx.try_recv() {
        ...
        egui::SidePanel::left("toolbar").show(ctx, |ui| {
            ...
            // Forcer l'onglet actif selon l'etat reseau en cours.
            ...
            // Selection manuelle uniquement quand aucune session n'est active.
                ...
                match self.mode {
                    BrushMode::Freehand | BrushMode::StraightLine => {
                        if response.dragged() {
                            ...
                            // Envoi incremental pour reduire la latence collaborative en Freehand.
                            if self.mode == BrushMode::Freehand && self.current_line.len() >= 2 {
                                ...
                                if a.distance_sq(b) > 0.25 {
                                    ...
                                    if self.server_running {
                                        let _ = server::publish_draw_line(DrawLineEvent::from_line(&segment));
